...
def get_background_normalized_separation_distance(network, sp, targets, seeds, distance="shortest", n_random=100,
                                                  n_node_in_bins=100, seed_to_equivalent_nodes=None, seed=None):
    if seed is not None:
        random.seed(seed)
...
def get_source_to_average_target_distance_random(sp, targets, nodes, distance, target_mean_and_std,
                                                 seed_to_equivalent_nodes, n_random=100):
    target_to_values = {}
...
def filter_network(g, degree_threshold=None, largest_connected_component=True):
    ...
    print
    "V,E filtered:", g_filtered.number_of_nodes(), g_filtered.number_of_edges()
    return g_filtered
...
# temp code
#res =  get_nodes_and_edges_from_sif_file('toy.sif')  
#print(res)
    ...
    if randomization_type == "random":
        ...
        for i in xrange(n_perturbation):
            ...
            while True:
                ...
                if n_trial > MAX_NUMBER_OF_TRIAL:
                    if debug:
                        print
                        "Warning: Max number of trials exceeded in perturbation ", i
                    break
                ...
                if debug:
                    print
                    source_id, target_id, " / ", new_source_id, new_target_id
                    print
                ...
                if debug:
                    print
                    "rm %s %s" % (source_id, target_id)
                edge_data = new_graph.get_edge_data(source_id, target_id)
                new_graph.remove_edge(source_id, target_id)
                if debug:
                    print
                    "add %s %s" % (new_source_id, new_target_id)
        ...
        for i in xrange(n_perturbation):
            ...
            while True:  # (len(nodes_by_degree[source_degree]) < 2 or len(available_neighbors) < 1):
                ...
                if debug:
                    print
                    "rm %d %d" % (source_id, target_id)
                    print
                    "rm %d %d" % (new_source_id, new_target_id)
                ...
                if debug:
                    print
                    "add %d %d" % (source_id, new_target_id)
                    print
                    "add %d %d" % (new_source_id, target_id)
...
def prune_non_seed_interactions_at_given_percentage(graph, percentage, reserved_nodes):
    ...
    if len(candidate_edges) < count:
        print
        "Warning: Pruning percentage is not achieved due to reserved nodes"
    return new_graph
        ...
        if len(common) > 1:
            ...
            for i, seed1 in enumerate(common):
                for j, seed2 in enumerate(common):
                    if i < j:
                        print
                        node_to_desc[seed1], node_to_desc[node], node_to_desc[seed2]
    # print linkers
